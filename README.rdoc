...
You'll also see, that there isn't any tests — the development was “dirty”, not TDD. Sorry for that, I hadn't much time during this holidays and were afraid of delays. Maybe, it could be left for the second big iteration — test suites and code refactoring.

Application matches the provided spec, but of course there is still many things to deal with — for example, different restrictions/validations, including ones based on regexp-patterns (e.g. for start_date and end_date), user authorization/authentification, administrative part to make some users able to add new types/statuses/other things, etcetera.
